...
public:
    ...
    // move
    unordered_multimap(unordered_multimap&& other) noexcept : table_() { table_.swap(other.table_); }
    unordered_multimap& operator=(unordered_multimap&& other) noexcept { table_.swap(other.table_); return *this; }
    ...
    iterator erase(const_iterator pos) {
        // Erase by matching the exact element value, not just the first key match.
        // For duplicate keys, find(pos->first) returns the first match which may not be pos.
        auto it = table_.find(pos->first);
        while (it != table_.end()) {
            if (&(*it) == &(*pos)) {
                table_.erase(it);
                return end();
