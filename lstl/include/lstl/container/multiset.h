/**
 * @file    multiset.h
 * @brief   Ordered multiset backed by red-black tree. Allows duplicate keys.
 * @author  lstl team
 * @date    2025
 * @ingroup container
 */
...
public:
    ...
    multiset(std::initializer_list<value_type> il) : tree_() {
    ...
    size_type erase(const key_type& k) {
        auto lb = tree_.lower_bound(k); auto ub = tree_.upper_bound(k);
        size_type n = 0;
