...
AsyncLogChannel* AsyncLogWriter::ensureThreadChannel() {
    if (t_channel) return t_channel;
    static thread_local AsyncLogChannel::ptr tls_channel;
    if (!tls_channel) {
        tls_channel = std::make_shared<AsyncLogChannel>();
        registerChannel(tls_channel);
    }
    return t_channel;
}

void AsyncBridgeLogAppender::log(std::shared_ptr<Logger> logger,
                                  LogLevel::Level level, LogEvent::ptr event) {
    (void)logger;
    auto* ch = AsyncLogWriter::GetInstance().ensureThreadChannel();
    if (!ch) return;
    std::string msg = event->getSS().str();
    ch->write(event->getTime(), event->getThreadId(),
              static_cast<uint32_t>(event->getFiberId()),
