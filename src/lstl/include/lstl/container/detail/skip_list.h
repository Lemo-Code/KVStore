...
struct skip_list_node {
    ...
    static skip_list_node** forward_ptr(skip_list_node* node) {
        // Must use the same aligned offset as forward_at() to avoid UB
        const size_t align = alignof(skip_list_node*);
        const size_t aligned_sz = (sizeof(skip_list_node) + align - 1) & ~(align - 1);
        return reinterpret_cast<skip_list_node**>(
            reinterpret_cast<char*>(node) + aligned_sz);
    }
};
...
public:
    ...
    skip_list_iterator& operator++() {
        node_ = skip_list_node<Value>::forward_ptr(node_)[0];
        return *this;
    }
