...
public:
    ...
    list(std::initializer_list<T> il) : sentinel_(), count_(0) {
        for (auto& x : il) push_back(x);
    }

    /** @brief  Copy constructor. */
    list(const list& other) : sentinel_(), count_(0) {
        for (auto& x : other) push_back(x);
    }

    /** @brief  Move constructor — transfers nodes in O(1). */
    list(list&& other) noexcept : sentinel_(), count_(0) {
        if (!other.empty()) {
