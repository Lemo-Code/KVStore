...
int64_t Socket::getSendTimeout() {
  lemo::io::FdContext::ptr ctx = lemo::io::FdManager::Instance().get(sock_);
  if (ctx) {
...
Socket::ptr Socket::accept() {
  if (lemo::io::is_hook_enable()) {
    lemo::io::FdManager::Instance().get(sock_, true);
  }
...
bool Socket::init(int sock) {
  if (lemo::io::is_hook_enable()) {
    lemo::io::FdContext::ptr ctx =
        lemo::io::FdManager::Instance().get(sock, true);
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
  ...
  } else if (lemo::io::connectWithTimeout(sock_, addr->getAddr(), addr->getAddrLen(),
                                timeout_ms) != 0) {
    close();
    return false;
  }
...
bool Socket::cancelRead() {
  lemo::io::IOManager* iom = lemo::io::IOManager::GetThis();
  return iom && iom->cancelEvent(sock_, lemo::io::IOManager::READ);
}

bool Socket::cancelWrite() {
  lemo::io::IOManager* iom = lemo::io::IOManager::GetThis();
  return iom && iom->cancelEvent(sock_, lemo::io::IOManager::WRITE);
}
