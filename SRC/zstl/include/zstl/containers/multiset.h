// zstl multiset — ordered set with duplicate keys, backed by red-black tree
//
// A sorted associative container that contains a set of keys.
// Multiple elements with the same key are allowed and stored in
...
public:
    ...
    size_type erase(const Key& key) {
        ...
        size_type count = 0;
        auto it = find(key);
        while (it != end() && !tree_.key_comp()(key, *it) && !tree_.key_comp()(*it, key)) {
            it = erase(it);
            ++count;
        }
        return count;
