...
void test_add_timer_callback() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_timer_cb"));
  sch->start();
...
void test_fiber_sleep_ms() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_fiber_sleep"));
  sch->start();

  std::atomic<int> phase{0};
  sch->schedule([&phase]() {
    phase.store(1);
    net::Fiber::SleepMs(60);
    phase.store(2);
  });
...
void test_cancel_timer() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_cancel_timer"));
  sch->start();
...
void test_multiple_timers() {
  net::Scheduler::ptr sch(
      new net::Scheduler(2, false, "test_multi_timer"));
  sch->start();
