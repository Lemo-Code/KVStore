...
void test_recurring_timer() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_recurring"));
  sch->start();
...
void test_refresh_reset() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_refresh_reset"));
  sch->start();
  ...
  NET_CHECK(timer->refresh());

  std::this_thread::sleep_for(std::chrono::milliseconds(120));
  NET_CHECK(fired.load() == 0);

  NET_CHECK(timer->reset(40, true));

  wait_until([&fired]() { return fired.load() >= 1; });
  NET_CHECK(fired.load() == 1);
...
void test_long_delay() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_long_delay"));
  sch->start();
...
void test_condition_timer() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_cond_timer"));
  sch->start();
...
void test_timer_ptr_cancel() {
  net::Scheduler::ptr sch(
      new net::Scheduler(1, false, "test_timer_ptr_cancel"));
  sch->start();

  std::atomic<int> fired{0};
  net::Timer::ptr timer =
      sch->addTimerPtr(80, [&fired]() { fired.store(1); });
  NET_CHECK(timer->cancel());
