 ...
 *
 * 运行：
 *   bin/net/bench_echo_server --mode local
...
uint16_t GetBoundPort(const net::Socket::ptr& listen) {
  net::Address::ptr addr = listen->getLocalAddress();
  net::IPv4Address::ptr v4 =
      std::dynamic_pointer_cast<net::IPv4Address>(addr);
  if (v4) {
    return v4->getPort();
  }
  net::IPv6Address::ptr v6 =
      std::dynamic_pointer_cast<net::IPv6Address>(addr);
  if (v6) {
    return v6->getPort();
  }
  return 0;
...
bool EchoSession(const net::Socket::ptr& sock, size_t buf_size) {
  std::vector<char> buf(buf_size);
  while (true) {
    const int n = sock->recv(buf.data(), buf.size());
    if (n <= 0) {
      break;
    }
    int sent = 0;
    while (sent < n) {
      const int w = sock->send(buf.data() + static_cast<size_t>(sent),
...
net::Socket::ptr MakeListenSocket(const std::string& host, uint16_t port) {
  net::Socket::ptr listen = net::Socket::CreateTCPSocket();
  net::IPv4Address::ptr addr = net::IPv4Address::Create(host.c_str(), port);
...
void StartAcceptLoop(net::IOManager* iom, const net::Socket::ptr& listen,
                     const std::atomic<bool>& stop, size_t buf_size) {
  while (!stop.load(std::memory_order_acquire)) {
    net::Socket::ptr client = listen->accept();
    if (!client) {
...
Result RunClientBench(net::IOManager* iom, const std::string& host,
  ...
  for (int i = 0; i < cfg.connections; ++i) {
    iom->schedule([host, port, cfg, &done]() {
      ...
      net::Socket::ptr sock = net::Socket::CreateTCPSocket();
      net::IPv4Address::ptr addr =
          net::IPv4Address::Create(host.c_str(), port);
...
Result RunLocalBench(const Config& cfg) {
  net::IOManager iom(static_cast<size_t>(cfg.threads), true, "echo_local");
  net::Socket::ptr listen = MakeListenSocket("127.0.0.1", 0);
  if (!listen) {
...
void RunServer(const Config& cfg) {
  ...
  net::IOManager iom(static_cast<size_t>(cfg.threads), false, "echo_server");
  net::Socket::ptr listen = MakeListenSocket("0.0.0.0", cfg.port);
  if (!listen) {
...
void RunClient(const Config& cfg) {
  net::IOManager iom(static_cast<size_t>(cfg.threads), true, "echo_client");
  if (cfg.port == 0) {
