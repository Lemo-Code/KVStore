/**
 * @file test_async_tcp.cc
 * @brief 异步 IO/DB 模式 TcpServer 集成：PING / SET / GET
 */
...
void test_async_tcp() {
  ...
  settings.io_threads = 2;

  ledis::LedisServer server(settings);
  LEDIS_CHECK(server.start());
  LEDIS_CHECK(server.engine().dbWorkerRunning());
  ...
  server.runtime()->iom().schedule([port, &done]() {
    ...
    set.args.push_back(ledis::Sds("async_k"));
    set.args.push_back(ledis::Sds("async_v"));
    ...
    get.args.push_back(ledis::Sds("async_k"));
    sendCommand(sock, get);
    auto val = readOneResponse(sock);
    LEDIS_CHECK(val.value.bulk.str() == "async_v");

    sock->close();
    done.store(1);
  });

  LEDIS_CHECK(waitUntil(&done, 1, 8000));
  server.stop();
}
