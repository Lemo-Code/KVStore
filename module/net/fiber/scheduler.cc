...
Scheduler::Scheduler(size_t threads, bool use_caller, const std::string& name)
  ...
  if (use_caller) {
    ...
    root_thread_ = static_cast<int>(GetThreadId());
...
void Scheduler::start() {
  ...
  for (size_t i = 0; i < thread_count_; ++i) {
    threads_[i].reset(new Thread([this] { this->run(); },
...
size_t Scheduler::resolveProcessorId() const {
  const int self_tid = static_cast<int>(GetThreadId());
...
bool Scheduler::fetchTask(ScheduleTask& task, size_t proc_id) {
  const int self_thread = static_cast<int>(GetThreadId());
  ScheduleTask candidate;
...
void Scheduler::switchTo(int thread) {
  ...
  if (Scheduler::GetThis() == this) {
    if (thread == -1 || thread == static_cast<int>(GetThreadId())) {
      return;
    }
  }
