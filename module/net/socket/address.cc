...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
  ...
  rt->addr_.sin_port = net::byteswapOnLittleEndian(port);
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
  ...
  addr_.sin_port = net::byteswapOnLittleEndian(port);
  addr_.sin_addr.s_addr = net::byteswapOnLittleEndian(address);
}

IPv4Address::IPv4Address(const sockaddr_in& addr) { addr_ = addr; }

std::ostream& IPv4Address::insert(std::ostream& os) const {
  uint32_t addr = net::byteswapOnLittleEndian(addr_.sin_addr.s_addr);
  os << ((addr >> 24) & 0xff) << "." << ((addr >> 16) & 0xff) << "."
     << ((addr >> 8) & 0xff) << "." << (addr & 0xff);
  os << ":" << net::byteswapOnLittleEndian(addr_.sin_port);
  return os;
}
...
IPAddress::ptr IPv4Address::broadcastAddress(uint32_t prefix_len) {
  ...
  baddr.sin_addr.s_addr |=
      net::byteswapOnLittleEndian(CreateMask<uint32_t>(prefix_len));
...
void IPv4Address::setPort(uint16_t v) {
  addr_.sin_port = net::byteswapOnLittleEndian(v);
}

uint16_t IPv4Address::getPort() const {
  return net::byteswapOnLittleEndian(addr_.sin_port);
}
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
  ...
  rt->addr_.sin6_port = net::byteswapOnLittleEndian(port);
...
IPv6Address::IPv6Address(const uint8_t address[16], uint16_t port) {
  memset(&addr_, 0, sizeof(addr_));
  addr_.sin6_family = AF_INET6;
  addr_.sin6_port = net::byteswapOnLittleEndian(port);
...
std::ostream& IPv6Address::insert(std::ostream& os) const {
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    os << std::hex << (int)net::byteswapOnLittleEndian(addr[i]) << std::dec;
  }
  ...
  os << "]:" << net::byteswapOnLittleEndian(addr_.sin6_port);
  return os;
}
...
void IPv6Address::setPort(uint16_t v) {
  addr_.sin6_port = net::byteswapOnLittleEndian(v);
}

uint16_t IPv6Address::getPort() const {
  return net::byteswapOnLittleEndian(addr_.sin6_port);
}
