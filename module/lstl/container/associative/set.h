...
class set {
 ...
 public:
  ...
  set(set&& other) throw() : tree_(lstl::move(other.tree_)) {}
...
inline bool operator==(const set<Key, Compare, Alloc>& a, const set<Key, Compare, Alloc>& b) {
...
template <typename Key, typename Compare, typename Alloc>
inline void swap(set<Key, Compare, Alloc>& a, set<Key, Compare, Alloc>& b) {
  a.swap(b);
}
