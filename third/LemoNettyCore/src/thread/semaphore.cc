...
bool Semaphore::tryWait() { return sem_trywait(&semaphore_) == 0; }
...
bool Semaphore::timedWait(uint64_t timeout_ms) {
  ...
  const uint64_t nsec =
      static_cast<uint64_t>(ts.tv_nsec) + (timeout_ms % 1000) * 1000000ull;
  ts.tv_sec +=
      static_cast<time_t>(timeout_ms / 1000 + nsec / 1000000000ull);
