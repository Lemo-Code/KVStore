  ...
  if (timeout_ms != UINT64_MAX) {
    ...
    timer = iom->addConditionTimer(
        ...
        [iom, fd, winfo]() {
          ...
          t->cancelled = ETIMEDOUT;
          iom->cancelEvent(fd, lemo::io::IOManager::WRITE);
        },
        std::weak_ptr<void>(), false);
  }

  if (iom->addEvent(fd, lemo::io::IOManager::WRITE) != 0) {
    if (timer) {
      timer->cancel();
    }
    return -1;
...
int accept(int sockfd, struct sockaddr* addr, socklen_t* addrlen) {
  const int fd = static_cast<int>(
      do_io(sockfd, accept_f, lemo::io::IOManager::READ, SO_RCVTIMEO, addr, addrlen));
  if (fd >= 0) {
    lemo::io::FdManager::Instance().get(fd, true);
  }
  return fd;
...
ssize_t read(int fd, void* buf, size_t count) {
  return do_io(fd, read_f, lemo::io::IOManager::READ, SO_RCVTIMEO, buf, count);
}

ssize_t readv(int fd, const struct iovec* iov, int iovcnt) {
  return do_io(fd, readv_f, lemo::io::IOManager::READ, SO_RCVTIMEO, iov, iovcnt);
}

ssize_t recv(int sockfd, void* buf, size_t len, int flags) {
  return do_io(sockfd, recv_f, lemo::io::IOManager::READ, SO_RCVTIMEO, buf, len,
               flags);
}

ssize_t recvfrom(int sockfd, void* buf, size_t len, int flags,
                 struct sockaddr* src_addr, socklen_t* addrlen) {
  return do_io(sockfd, recvfrom_f, lemo::io::IOManager::READ, SO_RCVTIMEO, buf, len,
                flags, src_addr, addrlen);
}

ssize_t recvmsg(int sockfd, struct msghdr* msg, int flags) {
  return do_io(sockfd, recvmsg_f, lemo::io::IOManager::READ, SO_RCVTIMEO, msg,
                flags);
}

ssize_t write(int fd, const void* buf, size_t count) {
  return do_io(fd, write_f, lemo::io::IOManager::WRITE, SO_SNDTIMEO, buf, count);
}

ssize_t writev(int fd, const struct iovec* iov, int iovcnt) {
  return do_io(fd, writev_f, lemo::io::IOManager::WRITE, SO_SNDTIMEO, iov, iovcnt);
}

ssize_t send(int sockfd, const void* buf, size_t len, int flags) {
  return do_io(sockfd, send_f, lemo::io::IOManager::WRITE, SO_SNDTIMEO, buf, len,
               flags);
}

ssize_t sendto(int sockfd, const void* buf, size_t len, int flags,
               const struct sockaddr* dest_addr, socklen_t addrlen) {
  return do_io(sockfd, sendto_f, lemo::io::IOManager::WRITE, SO_SNDTIMEO, buf, len,
               flags, dest_addr, addrlen);
}

ssize_t sendmsg(int sockfd, const struct msghdr* msg, int flags) {
  return do_io(sockfd, sendmsg_f, lemo::io::IOManager::WRITE, SO_SNDTIMEO, msg,
               flags);
}
