...
struct EpollContext::FdContext {
  ...
  EventContext& getContext(EpollContext::Event event) {
    switch (event) {
      case EpollContext::READ:
        return read;
      case EpollContext::WRITE:
        return write;
      default:
        assert(false);
    }
  ...
  void triggerEvent(EpollContext::Event event) {
    assert(events & event);
    events = static_cast<EpollContext::Event>(events & ~event);
  ...
  void triggerEventRunnext(EpollContext::Event event) {
    assert(events & event);
    events = static_cast<EpollContext::Event>(events & ~event);
    EventContext& ctx = getContext(event);
    if (ctx.cb) {
  ...
  EventContext write;
  int fd = 0;
  EpollContext::Event events = EpollContext::NONE;
  MutexType mutex;
  EpollContext* owner = nullptr;
};

EpollContext::Stats EpollContext::stats() const {
  Stats s;
...
void EpollContext::cancelAllEvents() {
...
void EpollContext::contextResize(size_t size) {
...
int EpollContext::addEvent(int fd, Event event, std::function<void()> cb) {
...
bool EpollContext::delEvent(int fd, Event event) {
  thread::RWMutex::ReadLock lock(mutex_);
  if (static_cast<int>(fd_contexts_.size()) <= fd) {
    return false;
  }
...
bool EpollContext::cancelEvent(int fd, Event event) {
  thread::RWMutex::ReadLock lock(mutex_);
  if (static_cast<int>(fd_contexts_.size()) <= fd) {
    return false;
  }
...
bool EpollContext::cancelAll(int fd) {
  thread::RWMutex::ReadLock lock(mutex_);
  if (static_cast<int>(fd_contexts_.size()) <= fd) {
    return false;
  }
...
void EpollContext::poll(uint64_t timeout_ms) {
